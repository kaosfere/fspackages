class B747_8_FMC_MainDisplay extends Boeing_FMC {
    ...
    Init() {
        ...
        this.onProg = () => { FMCProgPage.ShowPage(this); };
    ...
    _computeV1Speed() {
        ...
        dWeightCoeff = 0.7 + (1.17 - 0.7) * dWeightCoeff;
    ...
    _computeV2Speed() {
        ...
        let dWeightCoeff = (this.getWeight(true) - 550) / (1000 - 550);
        dWeightCoeff = Utils.Clamp(dWeightCoeff, 0, 1);
        dWeightCoeff = 1.07 + (1.43 - 1.07) * dWeightCoeff;
        let flapsHandleIndex = SimVar.GetSimVarValue("FLAPS HANDLE INDEX", "Number");
        let temp = SimVar.GetSimVarValue("AMBIENT TEMPERATURE", "celsius");
        let index = this._getIndexFromTemp(temp);
        console.log("Index From Temp = " + index);
    ...
    getManagedApproachSpeed(flapsHandleIndex = NaN) {
        if (isNaN(flapsHandleIndex)) {
            flapsHandleIndex = SimVar.GetSimVarValue("FLAPS HANDLE INDEX", "Number");
        }
        let dWeight = (this.getWeight(true) - 440) / (1012 - 440);
        let min = 205;
        let max = 266;
    ...
    getSlatApproachSpeed(useCurrentWeight = true) {
        ...
        let dWeight = ((useCurrentWeight ? this.getWeight(true) : this.zeroFuelWeight) - 258.4) / (447.5 - 258.4);
        return 141 + 48 * dWeight;
    }
